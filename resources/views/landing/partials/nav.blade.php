...
<nav class="navbar" id="navbar">
    <div class="container">
        <div class="nav-inner">
            <a href="{{ $logoUrl ?? ($homeUrl ?? route('home')) }}" class="nav-logo">
            ...
            <ul class="nav-links">
                @foreach ($navLinks ?? $defaultNavLinks as $link)
                    <li><a href="{{ $link['url'] }}" @if (($link['active'] ?? false)) class="active" @endif>{{ $link['label'] }}</a></li>
                @endforeach
            </ul>

            <div class="nav-actions">
                <a href="{{ $settings->header_btn1_url }}" class="nav-wa" target="_blank" rel="noopener">
                    ...
                    {{ $settings->header_btn1_title }}
                </a>
                @if (($activePage ?? 'home') === 'home')
                    <button class="nav-btn" data-scroll-to-form>{{ $settings->header_btn2_title }}</button>
                @else
                    <a href="{{ ($homeUrl ?? route('home')) . '#trial-form' }}" class="nav-btn">{{ $settings->header_btn2_title }}</a>
                @endif
            </div>
    ...
    <div class="mobile-menu" id="mobileMenu">
        ...
        <div class="mobile-btns">
            ...
            @else
                <a href="{{ ($homeUrl ?? route('home')) . '#trial-form' }}" class="btn-primary">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                    {{ $settings->header_btn2_title }}
                </a>
            @endif
